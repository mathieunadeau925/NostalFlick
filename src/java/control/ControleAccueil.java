...
@WebServlet(name = "ControleAccueil", urlPatterns = {"/ControleAccueil"})
public class ControleAccueil extends HttpServlet {

    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        HttpSession session = request.getSession();
        String connectedClient;
        String connectedEmploye;
        String dest = "/WEB-INF/accueil/homepage.jsp";
        try {
            if (session.getAttribute("connected") != null) {
                connectedClient = (String) session.getAttribute("connected");
            ...
            if (session.getAttribute("connectedEmp") != null) {
                connectedEmploye = (String) session.getAttribute("connectedEmp");
